...
test('REV23 exports core scoring and environment helpers',()=>{
  ...
  assert.equal(pkg.appRevision,23);
});
...
test('health reports REV23, marine support and NVE configuration state',async t=>{
  ...
  assert.equal(health.ok,true);assert.equal(health.version,'v11-rev23');assert.equal(health.revision,'REV 23');assert.equal(health.marine,true);assert.equal(typeof health.nveHydApiConfigured,'boolean');
});

test('PWA shell exposes marine, NVE, catch export and owned-lure helpers',()=>{
  const html=fs.readFileSync(path.join(root,'index.html'),'utf8');
  const js=fs.readFileSync(path.join(root,'app.js'),'utf8');
  assert.match(html,/REV 23/);assert.match(html,/marineCard/);assert.match(html,/hydrologyCard/);assert.match(html,/conditionToggle/);assert.match(html,/boatRampToggle/);assert.match(html,/exportGpx/);assert.match(html,/ownedLures/);
...
test('asset versions and service worker cache are REV23',()=>{
  const html=fs.readFileSync(path.join(root,'index.html'),'utf8');
  const sw=fs.readFileSync(path.join(root,'sw.js'),'utf8');
  assert.match(html,/app\.js\?v=23\.0/);assert.match(html,/fishing-insights\.js\?v=23\.0/);assert.match(html,/style\.css\?v=23\.0/);
  assert.match(sw,/fiste-guiden-rev23/);assert.doesNotMatch(sw,/rev22/);assert.match(sw,/\/api\//);
});
...
test('owned lure source still contains 14 photographed lure groups',()=>{
  const data=JSON.parse(fs.readFileSync(path.join(root,'data','user-lures.json'),'utf8'));
  assert.equal(data.lures.length,14);assert.ok(data.lures.every(item=>item.image&&item.species&&item.waterTypes));
});
