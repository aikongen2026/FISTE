...
test('REV26 exports core scoring and environment helpers',()=>{
  ...
  assert.equal(pkg.appRevision,26);
});
...
test('REV26 base radius creates a search box centered on base',()=>{
...
test('REV26 accepts all-sea selection without treating it as a single species',()=>{
...
test('health reports REV26, marine support and NVE configuration state',async t=>{
  ...
  assert.equal(health.ok,true);assert.equal(health.version,'v11-rev26');assert.equal(health.revision,'REV 26');assert.equal(health.marine,true);assert.equal(typeof health.nveHydApiConfigured,'boolean');
});

test('PWA shell exposes marine, NVE, catch export and owned-lure helpers',()=>{
  const html=fs.readFileSync(path.join(root,'index.html'),'utf8');
  const js=fs.readFileSync(path.join(root,'app.js'),'utf8');
  assert.match(html,/REV 26/);assert.match(html,/multiSpeciesCard/);assert.match(html,/Ingen – vis alle sjøarter/);assert.match(html,/marineCard/);assert.match(html,/hydrologyCard/);assert.match(html,/conditionToggle/);assert.match(html,/boatRampToggle/);assert.match(html,/exportGpx/);assert.match(html,/ownedLures/);
  assert.match(js,/applyPersonalRanking/);assert.match(js,/clearBasePoint/);assert.match(js,/focusBaseRadius/);assert.match(js,/speciesColors/);assert.match(js,/exportCatchGpx/);assert.match(js,/analysisCacheKey/);assert.match(js,/renderConditionVectors/);assert.match(js,/loadBoatRamps/);assert.match(js,/loadHydrologyAtCenter/);
});

test('asset versions and service worker cache are REV26',()=>{
  const html=fs.readFileSync(path.join(root,'index.html'),'utf8');
  const sw=fs.readFileSync(path.join(root,'sw.js'),'utf8');
  assert.match(html,/app\.js\?v=26\.0/);assert.match(html,/fishing-insights\.js\?v=26\.0/);assert.match(html,/style\.css\?v=26\.0/);
  assert.match(sw,/fiste-guiden-rev26/);assert.doesNotMatch(sw,/rev22/);assert.match(sw,/\/api\//);
});


test('REV26 detailed fishing map uses Kartverket depth WMS and no EMODnet color overlay',()=>{
  const html=fs.readFileSync(path.join(root,'index.html'),'utf8');
  const js=fs.readFileSync(path.join(root,'app.js'),'utf8');
...
test('REV26 personal lure box contains many individual photographed candidates',()=>{
